  ...
  return (
    <section className="daily-steps-card">
      ...
      <div className="daily-steps-meta">
        ...
        <strong className="daily-steps-remaining">
...
function HomePage({ userId, season, onNavigate, onRegister, done }: { userId: string; season: CurrentSeason | null; onNavigate: (page: Page) => void; onRegister: () => void; done: boolean }) {
  ...
  const [dailyMinutesGoal, setDailyMinutesGoal] = useState(30)
  useEffect(() => { if (!supabase) return; const load = async () => { const { data: seasons } = await supabase.from('seasons').select('id, name, start_date, end_date, status, rules').in('status', ['registration', 'active']).order('start_date', { ascending: false }); const today = moveToday(); const season = (seasons ?? []).find(item => item.start_date <= today && item.end_date >= today) as { id: string; name: string } | undefined; const displaySeason = season ?? (seasons ?? []).find(item => item.status === 'registration' && item.start_date > today) as { id: string; name: string } | undefined; setSeasonName(displaySeason?.name ?? '');
    const configuredStepsGoal = Number((displaySeason as { rules?: { daily_steps?: number } } | undefined)?.rules?.daily_steps ?? 8000);
    const configuredMinutesGoal = Number((displaySeason as { rules?: { daily_minutes?: number } } | undefined)?.rules?.daily_minutes ?? 30);
    setDailyStepsGoal(configuredStepsGoal);
    setDailyMinutesGoal(configuredMinutesGoal);
...
function RankingPage({ userId }: { userId: string }) {
  ...
  useEffect(() => {
    ...
    const loadRanking = async () => {
      ...
      if (seasonError) {
        setError('Não foi possível carregar a temporada.')
        setRows([])
        setLoading(false)
        return
      }
      ...
      for (const item of scoreResult.data ?? []) {
        ...
        existing.consistency_points += Number(row.consistency_points ?? 0)
        existing.evolution_points += Number(row.evolution_points ?? 0)
        existing.volume_points += Number(row.volume_points ?? 0)
        existing.bonus_points += Number(row.bonus_points ?? 0)
        existing.completed_days += Number(row.completed_days ?? 0)
  ...
  return (
    <>
      <PageTitle
        eyebrow="PLACAR DA TEMPORADA"
        title="Quem está se movendo?"
        detail={
          seasonName
            ? `${seasonName} · pontuação oficial calculada e validada pelo Supabase.`
            : 'Pontuação oficial calculada e validada pelo Supabase.'
        }
      />
      ...
      <div className="your-position">
        ...
        <div className="gap-copy">
          {personAbove ? (
            <>
              <strong>
                {pointsToAbove.toLocaleString('pt-BR')} pts
              </strong>
              <span>
                para alcançar a posição acima
              </span>
            </>
          ) : position === 1 ? (
            <>
              <strong>No topo</strong>
              <span>continue consistente</span>
            </>
          ) : (
            <>
              <strong>—</strong>
              <span>aguardando sua classificação</span>
            </>
          )}
        </div>
      </div>
      ...
      <div className="leaderboard">
        {loading ? (
          ...
          <p className="admin-empty">
            Nenhum participante ativo encontrado nesta temporada.
          </p>
        ) : (
          rows.map((person, index) => (
            ...
            >
              ...
              <span className="rank-avatar">
                {person.profiles?.avatar_emoji || '✦'}
              </span>

              <div className="rank-person">
                <strong>
                  {person.profiles?.full_name ?? 'Participante'}
                </strong>

                <span>
                  <Flame size={13} />
                  {person.completed_days} dias
                  <i />
                  {person.consistency_points} pts consistência
                </span>
              </div>
      ...
      <div className="score-info">
        ...
        <div>
          <strong>Como funciona o placar?</strong>

          <p>
            Consistência, evolução e volume vêm de
            atividades validadas no servidor. Peso,
            IMC, gordura corporal, medidas e aparência
            não participam do ranking. Na premiação as
            categorias acumulam: quem vence mais de uma
            recebe todas.
          </p>
        </div>
...
function ActivitySessionForm({ season, activeSession, completedSession, onStarted, onCompleted, onCancelled, onDone }: { season?: CurrentSeason | null; activeSession: ActivitySession | null; completedSession: ActivitySession | null; onStarted: (session: ActivitySession) => void; onCompleted: (session: ActivitySession) => void; onCancelled: () => void; onDone: (session: ActivitySession) => void }) {
  ...
  const dailyMinutesGoal = Number(season?.rules?.daily_minutes ?? 30)
  ...
  const reachedGoal = elapsed >= dailyMinutesGoal * 60
  const start = async () => {
