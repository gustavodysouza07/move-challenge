...
function ActivitySessionForm({ season, activeSession, completedSession, onStarted, onCompleted, onDone }: { season?: CurrentSeason | null; activeSession: ActivitySession | null; completedSession: ActivitySession | null; onStarted: (session: ActivitySession) => void; onCompleted: (session: ActivitySession) => void; onDone: (session: ActivitySession) => void }) {
...
function RegisterModal({ season, activeSession, completedSession, onClose, onStarted, onCompleted, onDone }: { season: CurrentSeason | null; activeSession: ActivitySession | null; completedSession?: ActivitySession | null; onClose: () => void; onStarted: (session: ActivitySession) => void; onCompleted?: (session: ActivitySession) => void; onDone: (session: ActivitySession) => void }) { return <div className="modal-backdrop" onMouseDown={activeSession ? undefined : onClose}><div className="register-modal" onMouseDown={e => e.stopPropagation()}><div className="modal-head"><div><span className="eyebrow">{activeSession ? 'SESSÃO ATIVA' : 'NOVO MOVIMENTO'}</span><h2>{activeSession ? 'Continue no seu ritmo.' : 'Comece seu movimento.'}</h2></div>{!activeSession && <button className="icon-button" onClick={onClose}><X size={20} /></button>}</div><ActivitySessionForm season={season} activeSession={activeSession} completedSession={completedSession ?? null} onStarted={onStarted} onCompleted={onCompleted ?? (() => undefined)} onDone={onDone} /></div></div> }
